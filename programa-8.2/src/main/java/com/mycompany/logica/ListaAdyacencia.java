...
public class ListaAdyacencia implements Serializable {
    ...
    public Nodo buscarSecuencial(String nombre) {
         System.out.println (grafo.get(nombre));
...
public String mostrarElementosPorClaseYLinea(String tipoTransporte, int lineaSeleccionada) {
 ...
 public void eliminarNodo(String nombre) {
    Nodo nodoAEliminar = grafo.get(nombre);
    ...
    System.out.println("Eliminando nodo: " + nombre);

    // Eliminar el nodo de las adyacencias de otros nodos
    for (Nodo nodo : nodos) {
        if (nodo.adyacentes.remove(nodoAEliminar)) {
            System.out.println("Eliminado de las adyacencias del nodo: " + nodo.dato.toString());
        }
    }

    // Eliminar del HashMap y de la lista de nodos
    grafo.remove(nombre);
